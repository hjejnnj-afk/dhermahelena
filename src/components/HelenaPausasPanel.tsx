...
export function HelenaPausasPanel({ usuarioAtual }: HelenaPausasPanelProps) {
  ...
  async function handlePausar() {
    setFeedback(null);
    // normaliza: só dígitos, adiciona 55 se vier sem DDI
    let sender = telefone.replace(/\D/g, "");
    if (!sender) {
      setFeedback("Informe o número de WhatsApp (com DDD).");
      return;
    }
    if (sender.length === 10 || sender.length === 11) sender = `55${sender}`;
    if (sender.length < 12 || sender.length > 13) {
      setFeedback("Número inválido — use o formato DDD + número (ex.: 98 99110-3125).");
      return;
    }

    // O WhatsApp registra números BR antigos SEM o nono dígito no ID da
    // conversa. Pausamos as duas variantes pra garantir o match com o
    // sender que o fluxo n8n realmente consulta.
    const variantes = new Set<string>([sender]);
    if (sender.length === 13 && sender[4] === "9") {
      variantes.add(sender.slice(0, 4) + sender.slice(5)); // remove o nono dígito
    } else if (sender.length === 12) {
      variantes.add(`${sender.slice(0, 4)}9${sender.slice(4)}`); // adiciona o nono dígito
    }
